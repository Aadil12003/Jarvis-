...
if "client" not in st.session_state:
    st.session_state.client = OpenAI(api_key=OPENAI_API_KEY, base_url=OPENAI_BASE_URL)
...
with col1:
    st.markdown(
        f"<h2 style='color: {PRIMARY_COLOR}; margin-top: 0;'>💬 COMMAND INTERFACE</h2>",
        unsafe_allow_html=True,
    )
    ...
    with conversation_container:
        for i, msg in enumerate(st.session_state.conversation_history):
            if msg["role"] == "user":
                st.markdown(
                    f"<div class='message-box' style='border-left: 4px solid {SUCCESS_COLOR};'><strong>YOU:</strong><br>{msg['content']}</div>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f"<div class='message-box' style='border-left: 4px solid {PRIMARY_COLOR};'><strong>JARVIS:</strong><br>{msg['content']}</div>",
                    unsafe_allow_html=True,
                )

    # Input section
    st.markdown(
        f"<hr style='border-color: {PRIMARY_COLOR}; opacity: 0.3;'>",
        unsafe_allow_html=True,
    )

    user_input = st.text_input(
        "Enter Command:", placeholder="Type your command here...", key="command_input"
    )
    ...
    def execute_command(command_text):
        if command_text.strip():
            ...
            logger.info(f"User input: {command_text}")
...
with col2:
    st.markdown(
        f"<h3 style='color: {PRIMARY_COLOR};'>⚙️ CONTROL PANEL</h3>",
        unsafe_allow_html=True,
    )

    st.markdown(
        f"<div class='message-box'><strong>Status:</strong><br><span style='color: {SUCCESS_COLOR};'>● ONLINE</span></div>",
        unsafe_allow_html=True,
    )

    st.markdown(
        f"<div class='message-box'><strong>Mode:</strong><br><span style='color: {PRIMARY_COLOR};'>TEXT INPUT</span></div>",
        unsafe_allow_html=True,
    )

    st.markdown(
        f"<div class='message-box'><strong>Model:</strong><br><span style='color: {PRIMARY_COLOR};'>{MODEL}</span></div>",
        unsafe_allow_html=True,
    )

    st.markdown(
        f"<div class='message-box'><strong>Messages:</strong><br><span style='color: {WARNING_COLOR};'>{len(st.session_state.conversation_history)}</span></div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<h4 style='color: {ACCENT_COLOR};'>📋 Quick Commands</h4>",
        unsafe_allow_html=True,
    )

    if st.button("⌨️ Type 'Hello'"):
        execute_command("Hello, JARVIS!")

    if st.button("📁 Open Explorer"):
        execute_command("Open file explorer")

    if st.button("🔐 System Info"):
        execute_command("Show system information")

    st.divider()

    st.markdown(
        f"<h4 style='color: {WARNING_COLOR};'>⚠️ Voice Status</h4>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<span style='color: {ACCENT_COLOR}; font-size: 12px;'>Voice control disabled for Python 3.14 compatibility. Use text input above.</span>",
        unsafe_allow_html=True,
    )
    ...
    if OPENAI_API_KEY:
        st.markdown(
            f"<span style='color: {SUCCESS_COLOR}; font-size: 11px;'>✓ API KEY CONFIGURED</span>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f"<span style='color: {ACCENT_COLOR}; font-size: 11px;'>✗ API KEY MISSING - Add OPENAI_API_KEY to .env</span>",
            unsafe_allow_html=True,
        )
